...
export default async function AdminSettingsPage() {
  ...
  const directoryTitle = getSetting(
    "directory_title",
    "Currency Exchange Near Me"
  )
  const directorySubtitle = getSetting(
    "directory_subtitle",
    "Find physical currency exchange stores, bureaus, and local offices across the United States. Search by city or ZIP code to view listings, directions, ratings, and phone numbers."
  )

  // Popular Currency Settings
  const popcurrencyTitle = getSetting("popcurrency_title", "Popular Currencies")
  const popcurrencySubtitle = getSetting(
    "popcurrency_subtitle",
    "Explore the world's most used currencies."
  )
  const popularCurrencies = getSetting(
    "popular_currencies",
    "USD,EUR,JPY,GBP,CAD,AUD,CHF,CNY,INR,MXN,BRL,SGD,NZD,HKD,SEK,KRW"
  )
  ...
  const popularPairsTitle = getSetting(
    "popular_pairs_title",
    "Most Popular Currency Pairs"
  )
  const popularPairsSubtitle = getSetting(
    "popular_pairs_subtitle",
    "Quickly access real-time conversion rates and charts for high-traffic trading pairs."
  )
  const popularPairsGlobalTitle = getSetting(
    "popular_pairs_global_title",
    "Most Popular Currency Exchange Globally"
  )
  const popularPairsGlobalSubtitle = getSetting(
    "popular_pairs_global_subtitle",
    "Configure custom exchange pairs and display names in your admin settings."
  )

  // Live Exchange Range Settings
  const liveRatesTitle = getSetting("live_rates_title", "Live Exchange Rates")
  const liveRatesSubtitle = getSetting(
    "live_rates_subtitle",
  ...
  const factsSubtitle = getSetting(
    "facts_subtitle",
    "Did you know? Explore these interesting facts about foreign exchange markets and global currencies."
  )
  ...
  const mistakesTitle = getSetting(
    "mistakes_title",
    "8 Common Currency Exchange Mistakes to Avoid"
  )
  const mistakesSubtitle = getSetting(
    "mistakes_subtitle",
    "Keep these pitfalls in mind to secure the best rates and keep more of your funds during exchange transactions."
  )
  ...
  const regionsSubtitle = getSetting(
    "regions_subtitle",
    "Explore global currencies and economic indicators divided by continent."
  )
  ...
  return (
    <div className="space-y-8">
      ...
      <SettingsForms
        ...
        popularPairsStandard={popularPairsStandard}
        popularPairsTitle={popularPairsTitle}
        popularPairsSubtitle={popularPairsSubtitle}
        popularPairsGlobalTitle={popularPairsGlobalTitle}
        popularPairsGlobalSubtitle={popularPairsGlobalSubtitle}
